...
client identifier, and selected audiobook
library in `${XDG_CONFIG_HOME:-~/.config}/plextales/config.json`. The
...
Local configuration, progress, and cache operations are bound to opened directory file
...
PlexTales passes it to mpv as a file-local HTTP header over a user-owned Unix
...
`${XDG_CACHE_HOME:-~/.cache}/plextales`, whose size and age are
...
client identifier, selected library, and local audiobook progress in the same private configuration file.
While switched off, PlexTales stops playback and timeline reporting and does not
make Plex API, health, artwork, or library requests. Use `plextales logout` when
