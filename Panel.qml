...
Panel {
  id: root
  moduleName: "io.github.flathack.plextales"
  ipcTarget: "io.github.flathack.plextales"
  ...
  readonly property var keyboardHelp: [
    ...
    { keys: "Shift+Enter", action: "Play chapter next" },
  ...
  readonly property url helperUrl: Qt.resolvedUrl("bin/plextales")
  readonly property url book24Url: Qt.resolvedUrl("assets/book.svg")
  readonly property url book64Url: Qt.resolvedUrl("assets/book.svg")
  ...
  function nextSpeed() {
    ...
    var current = Number(player.speed || 1)
    for (var i = 0; i < speeds.length; i++)
      if (speeds[i] > current + 0.01) return speeds[i]
    return speeds[0]
  }
  ...
  WidgetButton {
    ...
    tooltipText: !root.plexConnected && root.configured ? "PlexTales · Plex connection off" : root.activeTrack
    ...
    onWheelMoved: function(delta) {
      if (root.activeTrack) root.control("seek", Math.max(0, Math.min(Number(root.player.duration) || 0,
        Number(root.player.position) + (delta > 0 ? -30 : 30))))
    }
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      RowLayout {
        ...
        ColumnLayout {
          ...
          Text {
            textFormat: Text.PlainText
            Layout.fillWidth: true
            text: root.activeTrack ? (root.activeTrack.album || root.activeTrack.title) : "PlexTales"
          ...
          Text {
            textFormat: Text.PlainText
            Layout.fillWidth: true
            text: root.activeTrack ? root.activeTrack.title + " · " + (root.activeTrack.artist || "")
              : (root.plexConnected ? "Choose an audiobook from your library" : "Connect Plex to play audiobooks")
            color: root.dim
            font.family: root.fontFamily
      ...
      TextField {
        ...
        Accessible.name: "Search Plex audiobooks"
        foreground: root.foreground
        font.family: root.fontFamily
        text: root.query
        maximumLength: 256
        ...
        PanelSlider {
          ...
          Keys.onPressed: function(event) {
            if (event.key === Qt.Key_Left || event.key === Qt.Key_Down) {
              root.control("seek", Math.max(0, Number(root.player.position) - 30)); event.accepted = true
            } else if (event.key === Qt.Key_Right || event.key === Qt.Key_Up) {
              root.control("seek", Math.min(Number(root.player.duration) || 0, Number(root.player.position) + 30)); event.accepted = true
      ...
      RowLayout {
        ...
        PanelActionButton {
          ...
          Accessible.name: tooltipText
          onClicked: root.control("seek", Math.max(0, Number(root.player.position) - 30))
        }
        PanelActionButton {
          size: Style.space(30)
          iconText: "\uf048"
        ...
        PanelActionButton {
          ...
          enabled: !!root.activeTrack
          Accessible.name: tooltipText
          onClicked: root.control("seek", Math.min(Number(root.player.duration) || 0, Number(root.player.position) + 30))
        }
    ...
    Column {
      ...
      RowLayout {
        ...
        Rectangle {
          ...
          Image {
            ...
            height: width
            visible: root.helpVisible || (!root.activeTrack && !headerCover.visible)
            source: root.helpVisible ? root.book64Url : root.book24Url
            fillMode: Image.PreserveAspectFit
            smooth: false
            mipmap: false
        ...
        ColumnLayout {
          ...
          Text {
            textFormat: Text.PlainText
            Layout.fillWidth: true
            text: root.helpVisible || !root.plexConnected ? "PlexTales" : (root.activeTrack ? (root.activeTrack.album || root.activeTrack.title) : "PlexTales")
            color: root.foreground
            font.family: root.fontFamily
          ...
          Text {
            ...
            text: root.helpVisible ? "Help · Keyboard map and player settings"
              ...
              : (root.activeTrack ? root.activeTrack.title + " · " + (root.activeTrack.artist || "") : (root.configured ? "Choose a book to play" : "Connect your Plex server")))
            color: root.dim
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
            elide: Text.ElideRight
        ...
        Column {
          ...
          Text {
            ...
            text: root.volumeMode === "system"
              ...
              : "The slider changes only PlexTales's local mpv player."
            color: root.dim
            font.family: root.fontFamily
            font.pixelSize: Style.font.caption
            wrapMode: Text.WordWrap
      ...
      Row {
        ...
        Row {
          ...
          spacing: Style.space(10)
          PanelActionButton {
            iconText: "\uf0e2"; tooltipText: "Back 30 seconds"; foreground: root.foreground; fontFamily: root.fontFamily
          ...
          PanelActionButton {
            iconText: "\uf01e"; tooltipText: "Forward 30 seconds"; foreground: root.foreground; fontFamily: root.fontFamily
            focusable: true; Accessible.name: tooltipText
            onClicked: root.control("seek", Math.min(Number(root.player.duration) || 0, Number(root.player.position) + 30))
          }
          PanelActionButton {
          ...
          PanelActionButton {
            size: Style.space(42)
            iconText: Number(root.player.speed || 1).toFixed(2).replace(/\.?0+$/, "") + "×"
            tooltipText: "Playback speed " + Number(root.player.speed || 1).toFixed(2).replace(/\.?0+$/, "") + "×"
            foreground: root.foreground; fontFamily: root.fontFamily; focusable: true
            Accessible.name: tooltipText
            onClicked: root.control("speed", root.nextSpeed())
          }
        }
      }
      ...
      RowLayout {
        ...
        PanelActionButton {
          ...
          iconText: "\uf04b"; tooltipText: "Continue book"; foreground: root.foreground; fontFamily: root.fontFamily; bordered: true
          focusable: true; Accessible.name: tooltipText
        ...
        delegate: CursorSurface {
          ...
          RowLayout {
            ...
            Text {
              ...
              text: mediaRow.modelData.type === "artist" ? mediaRow.modelData.leafCount + " books" : Model.formatTime(mediaRow.modelData.duration)
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
            }
            ...
            Row {
              ...
              spacing: Style.space(3)
              PanelActionButton {
                iconText: "\uf04b"; tooltipText: "Continue"; foreground: root.foreground; fontFamily: root.fontFamily
