...
class PlayerTests(unittest.TestCase):
    ...
    def test_shutdown_reports_timeline_and_quits_mpv(self):
        ...
        with mock.patch.object(player, "stop_timeline") as stop, \
             mock.patch.object(player, "mpv_running", side_effect=[True, True, False]), \
             mock.patch.object(player, "mpv_command") as command, \
             mock.patch.object(player, "status"):
    ...
    def test_timeline_request_posts_player_identity_and_position(self):
        ...
        self.assertEqual(headers["x-plex-product"], "PlexTales")
...
class RepositoryContractTests(unittest.TestCase):
    def test_manifest_and_entry_point(self):
        ...
        self.assertEqual(manifest["id"], "io.github.flathack.plextales")
    ...
    def test_mpris_client_name_is_a_valid_bus_component(self):
        helper = (ROOT / "bin" / "plextales").read_text(encoding="utf-8")
        self.assertIn('"--audio-client-name=PlexTales"', helper)
    ...
    def test_book_asset_is_present(self):
        artwork = (ROOT / "assets" / "book.svg").read_text(encoding="utf-8")
        self.assertIn('<svg xmlns="http://www.w3.org/2000/svg"', artwork)
