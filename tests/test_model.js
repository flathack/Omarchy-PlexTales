...
test("active or paused playback opens on the queue", () => {
  ...
  assert.equal(model.defaultView({ playing: false, paused: false, track: { title: "Saved" } }), "continue");
  assert.equal(model.defaultView(null), "continue");
});

test("transport actions queue while sliders coalesce to the newest intent", () => {
  const helper = "/plugin/bin/plextales";
  const previous = [
    [helper, "control", "next"],
