...
- Progress is stored in `${XDG_CONFIG_HOME:-~/.config}/plextales/progress.json`, outside the disposable artwork and library cache. It survives app restarts, cache cleanup, and Plex token refreshes. Progress and favorites are separated by Plex account, server, and library. When you open or continue a book, PlexTales checks Plex for a newer chapter played on another device and moves to that chapter. **Resume** refreshes up to ten recently saved books. Plex can provide a track offset when its music library has **Store track progress** enabled; otherwise PlexTales starts at the detected chapter boundary. Local bookmarks and notes remain local to this computer.
